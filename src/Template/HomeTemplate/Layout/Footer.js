...
export default function Footer() {
    let { arrRapPhim } = useSelector(state => state.QuanLyRapPhimReducer)

    let arrHeThongRap = _.map(arrRapPhim, (heThongRap) => _.pick(heThongRap, ["maHeThongRap", "tenHeThongRap", "logo"]))
    return (
        <footer className="text-center bg-gray-900 text-white">
            <div className="container px-6 mx-auto pt-6">
                <div className="flex justify-between mb-6">
                    ...
                    <div className='flex'>
                        {_.map(arrHeThongRap, (item, index) => {
                            return <div className='m-4' key={index}>
                                <img src={item.logo} className='w-8' />
                            </div>
                        })}
                    </div>
